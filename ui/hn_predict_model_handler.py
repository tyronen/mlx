...
from models.hn_predict import QuantileRegressionModel, ClassifierModel, CACHE_FILE

FULL_MODEL_PATH = os.getenv(
    "FULL_MODEL_PATH", "models/20250613_130611/best_model_5.pth"
)
CLASSIFIER_PATH = os.getenv(
    "CLASSIFIER_MODEL_PATH", "models/20250613_130611/best_model_1.pth"
)
REGRESSOR_PATH = os.getenv(
    "REGRESSION_MODEL_PATH", "models/20250613_130611/best_model_2.pth"
)
...
def load_double_model() -> tuple[ClassifierModel, QuantileRegressionModel]:
    ...
    device = hn_predict_utils.get_device()
...
class FullModelPredictor(BasePredictor):
    ...
    def analyze_feature_importance(self, data, top_k=10):
        features_num = torch.tensor(
            data["features_num"], dtype=torch.float32, requires_grad=True
        ).unsqueeze(0)
        # Load title embeddings (precomputed)
        title_embeddings = torch.tensor(
            data["embedding"], dtype=torch.float32, requires_grad=True
        ).unsqueeze(0)
        ...
        if features_num.grad is not None and title_embeddings.grad is not None:
            ...
            importance_pairs = list(
                zip(self.columns[: len(num_importance)], num_importance)
            )
