...
if __name__ == "__main__":
    ...
    train_class_dataloader = DataLoader(
        train_class_dataset, batch_size=BATCH_SIZE, shuffle=True
    )
    val_class_dataloader = DataLoader(
        val_class_dataset, batch_size=BATCH_SIZE, shuffle=False
    )
    ...
    for epoch in range(1, EPOCHS_CLASS + 1):
        ...
        print(
            f"✓ Classifier Epoch {epoch}: Train Loss {avg_train_loss:.4f}, Val Loss {avg_val_loss:.4f}, Val Acc {val_acc:.4f}"
        )
    ...
    train_reg_dataloader = DataLoader(
        train_reg_dataset, batch_size=BATCH_SIZE, shuffle=True
    )
    val_reg_dataloader = DataLoader(
        val_reg_dataset, batch_size=BATCH_SIZE, shuffle=False
    )
    ...
    for epoch in range(1, EPOCHS_REG + 1):
        ...
        median_preds = predictions[
            :, 2
        ]  # index 2 is 50th percentile for quantiles=[0.1, 0.25, 0.5, 0.75, 0.9]
        ...
        print(
            f"✓ Regressor Epoch {epoch}: Train Loss {avg_train_loss:.4f}, Val Loss {avg_val_loss:.4f}, MAE {mae:.4f}"
        )
